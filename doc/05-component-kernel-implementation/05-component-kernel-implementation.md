...
I will use a Map<String, Double> to represent water sources. The key is the name of the water source while the value is the amount of water in ounces for that source. In addition, a String is used to represent the name of the hydration goal and a double is used to represent the target amount of water to consume.

By convention, waterSources is not null, all keys are not null or empty strings. Likewise, all values in waterSources are not negative, goalName is not null or empty, and goalAmount is not negative.

By correspondence, waterSources represents the mapping of water source names to their respective amounts. GoalName represents the name of the hydration goal and goalAmount represents the target consumption amount of water.
